...
## 【ローカルで作業する場合はスキップ】SSHでAzureなどのリモート（テスト）環境に接続する場合
おまじない：
1. コマンドプロンプトとかターミナルとかを開く
2. `ssh <アカウント名>@<ssh先のFQDN or IPアドレス>` を実行
3. パスワードを聞かれるので、アカウントのパスワードを入れてEnter
4. 最後に何か聞かれたらyes

これでLinuxのプロンプトが出てきたらOK。
以後、コマンド実行はここでやります。

## Node.js と Web3.js のインストール（使う環境で**インストールされていない場合のみ実施**）
以下は Ubuntu／WSL2 を想定しています。macOS でもほぼ同じ。Windows の場合は [Foundry](https://getfoundry.sh/) に合わせて WSL2 前提としています。

1. `curl` のインストール確認  
   `curl -V` を実行して、バージョンが表示されることを確認します。

   もし `curl: command not found` と表示された場合は、以下を実行して `curl` をインストールしてください。
   ```shell
   sudo apt install curl
   ```

2. Node.js をインストール  
   [Nodesource](https://deb.nodesource.com/) のページにある以下コマンドでインストールします。
   ```shell
   curl -fsSL https://deb.nodesource.com/setup_22.x | sudo bash -
   sudo apt-get install -y nodejs
   ```
   の後に `node -v` でバージョンが表示されればOK。
   
   > 想定バージョンはv22.x系ですが、 `curl` コマンドが入ってないと `curl` 部分がスキップされたりして 18.x 系とかになるとかもあります。この場合は `sudo apt install curl` で `curl` を入れてから再度上記コマンドを試してみてください。

   > 最初から `curl` 部分も含めて上記コマンドが全部ちゃんと通ったら、おそらく `npm` も一緒に入っているはず……

3. npm をインストール（npmがインストールされていない場合）  
   まず `npm -v` を試してみて、バージョンが表示されない（`command not found` になる）場合だけ以下を実行します。
   ```shell
   sudo apt install npm
   ```
   の後に `npm -v` でバージョンが表示されればOK。

4. プロジェクトを用意  
   ```shell
   mkdir -p ~/web3js-demo && cd ~/web3js-demo
   npm init -y
   ```
   `npm init` は依存関係を管理する `package.json` を作成します。既存プロジェクトがあるならこの手順は不要です。

5. Web3.js をインストール  
   プロジェクト配下で（1つ前の手順通りなら `cd ~/web3js-demo` で `~/web3js-demo` に居るはず）以下を実行。
   ```shell
   npm install web3
   ```
   ここでは Node.js の [REPL](https://nodejs.org/en/learn/command-line/how-to-use-the-nodejs-repl) から利用しますが、`web3` は基本的にアプリごとのローカル依存として入れるのが安全です（グローバルに入れると別プロジェクトでバージョンが衝突する可能性があるので）。

6. 動作確認  
   ```shell
   node -e "const Web3=require('web3').default; console.log('web3 version:', Web3.version)"
   ```
   ここでバージョン番号が表示されれば、以降のサンプルコードをこの環境でそのまま試せます。

## Node.jsの立ち上げ
`node` コマンドを実行すると、以下のようになるはず（もしかするとバージョンは違うかも）。
```shell
$ node
...
2. 「Include Testnets」にチェックを入れて、イーサリアムのテストネット（今回は「Sepolia」）を検索
3. 「ChainID」が「11155111」と書いてある、「Ethereum Sepolia」の下矢印みたいなのを押して展開する
...
今回はSepoliaテストネットを使うので、「etherscan sepolia」あたりで検索すると[Sepolia Testnet Explorer]( https://sepolia.etherscan.io/ )というのが出てくるので、ここでアカウントアドレスを入れて検索します。

### FaucetでETHを取得
Faucet（蛇口）から、テスト用にETHを取得します。送金テストをこの後にやりたいので、まずは片方のアカウントにテスト用の残高を取得しましょう。

「Faucet　<ネットワーク名>」みたいな感じでググると出てきたりします。
今回は[Google Cloud]( https://cloud.google.com/application/web3/faucet/ethereum/sepolia )のものを使ってみましょう。

※タイミングによっては制限が出る可能性があります。その場合は他のFaucetサービスを探してみてください。

1. 「Select network」で「Ethereum Sepolia」を選択（↑のリンクから行くと、たぶんすでにこれが選択されている）
2. 「Wallet address or ENS name」に、アカウントアドレスを入れる（2つ作ったうちの1つだけでOK）
3. 「Get 0.05 Sepolia ETH」を押す（取得できる数量はタイミングで変更されている可能性あり）
4. しばらく待つと、「Transaction complete! Check your wallet address」と出る：
   ```
...
#### 準備（Node.js を立ち上げなおす場合）
そのまま進んでもOKですが、環境をリセットしたい場合、一度 `Ctrl+D` なんかで Node.js を終了して、再度 `node` コマンドで Node.js を立ち上げましょう。

Node.js を再度立ち上げたら、先ほどと同様に [ChainList]( https://chainlist.org/ ) でRPCノードも準備しなおす必要があります。
...
3. ネットワークを適宜設定します。（今回すでに作成しているアカウントはSepoliaテストネットのETHを持っていると思うので、Sepoliaテストネットにするのが良いと思います）

ここまでで、作ったアカウントをMetaMaskで扱えるようになります。

### MetaMaskのUIお試し
Etherscanから[WETH]( https://sepolia.etherscan.io/address/0x7b79995e5f793A07Bc00c21412e50Ecae098E7f9 )（Wrapped ETH）のコントラクトに接続して、MetaMaskを連携させつつ関数を呼び出してみましょう。
...
#### MetaMaskとアカウントの準備
1. MetaMaskで、SepoliaのETH残高が入っているアカウントを選んでおく
...
4. 【**注意：これをやると、SepoliaのETH残高が減ります。**やりたくない場合はやらなくても良いです】MetaMaskが呼び出されるので、ちゃんと受け入れられる内容になってたら「確認」
...
`window.ethereum`については[MetaMask公式のAPIドキュメント]( https://docs.metamask.io/wallet/reference/provider-api/ )に記載があります（これはブラウザの開発者ツールでコンソールでオブジェクトを確認できたりもします）。[かるでね氏のQiita記事]( https://qiita.com/cardene/items/9e796da49299f57bd5c7 )も参考になるかと思います。
